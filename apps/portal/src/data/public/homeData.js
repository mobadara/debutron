export const homeCohortBanner = {
  id: 'home-cohort-banner',
  title: 'Next Academic & Tech Cohort Begins March 2026',
  subtitle:
    'Secure your place in our admission cycle for exam mastery, data, software, and cloud pathways.',
  description:
    'Secure your place in our admission cycle for exam mastery, data, software, and cloud pathways.',
  ctaLabel: 'Apply Now',
  ctaLink: '/admissions',
  cta: { label: 'Apply Now', to: '/admissions', href: '/admissions' },
}

export const homeVisionMissionBlock = {
  id: 'home-vision-mission',
  eyebrow: 'Who We Are',
  title: 'Vision, Mission & Core Values',
  body:
    'Debutron Lab stands at the intersection of academic distinction and applied technology training. We are committed to developing learners who excel in examinations, thrive in modern digital careers, and lead with responsibility in their communities.',
  description:
    'Debutron Lab stands at the intersection of academic distinction and applied technology training. We are committed to developing learners who excel in examinations, thrive in modern digital careers, and lead with responsibility in their communities.',
  cards: [
    {
      title: 'Vision',
      body:
        'To become a premier African learning hub where classical academic rigor and future-ready technology skills shape transformative leaders.',
    },
    {
      title: 'Mission',
      body:
        'To deliver high-impact academic coaching, structured admissions support, and practical technology programs that produce measurable learner outcomes.',
    },
    {
      title: 'Core Values',
      values: [
        { label: 'Excellence', body: 'Consistent quality in teaching and delivery.' },
        { label: 'Integrity', body: 'Ethical guidance and transparent systems.' },
        { label: 'Innovation', body: 'Practical, modern, and industry-relevant methods.' },
        { label: 'Inclusion', body: 'Accessible opportunities for diverse learners.' },
      ],
    },
  ],
}

export const homeShowcaseBlock = {
  id: 'home-showcase',
  eyebrow: 'Applied Innovation',
  title: 'Student App Showcase',
  body:
    'See how our tech-track students translate classroom learning into real-world products, from full-stack applications to practical AI-powered solutions.',
  description:
    'See how our tech-track students translate classroom learning into real-world products, from full-stack applications to practical AI-powered solutions.',
  ctaLabel: 'Explore Student Projects',
  ctaLink: '/student-showcase',
  cta: { label: 'Explore Student Projects', to: '/student-showcase', href: '/student-showcase' },
}
