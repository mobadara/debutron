...
import { footerAcademicTechLinks, footerAssessmentInnovationLinks, footerContact } from '../../data/domains/marketingContent'
				...
				<div className="grid gap-10 md:grid-cols-4">
					...
					<div>
						...
						<ul className="space-y-2">
							{footerAcademicTechLinks.map((item) => (
								<li key={item.to}><Link className="text-sm text-gray-300 transition-colors hover:text-white" to={item.to}>{item.label}</Link></li>
							))}
						</ul>
					</div>
					...
					<div>
						...
						<ul className="space-y-2">
							{footerAssessmentInnovationLinks.map((item) => (
								<li key={item.to}>
									<Link className="text-sm text-gray-300 transition-colors hover:text-white" to={item.to}>
										{item.label}
										{item.badge ? (
											<span className="ml-2 rounded-sm bg-yellow-400 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-debutron-navy">{item.badge}</span>
										) : null}
									</Link>
								</li>
							))}
					...
					<div>
						...
						<div className="space-y-3 text-sm text-gray-300">
							...
							<p className="flex items-start gap-2">
								...
								<a className="transition-colors hover:text-white" href={footerContact.phoneHref}>{footerContact.phoneLabel}</a>
							</p>
							<p className="flex items-start gap-2">
								<FaEnvelope className="mt-0.5 text-gray-300" aria-hidden="true" />
								<a className="transition-colors hover:text-white" href={footerContact.emailHref}>{footerContact.emailLabel}</a>
							</p>
							<p className="flex items-start gap-2">
								<span className="mt-0.5 text-gray-300" aria-hidden="true">🌐</span>
								<a className="transition-colors hover:text-white" href={footerContact.websiteHref} target="_blank" rel="noopener noreferrer">{footerContact.websiteLabel}</a>
							</p>
						</div>
