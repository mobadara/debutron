...
function LatestNews() {
	...
	return (
		<section>
			<h2 className="mb-8 text-center font-serif text-3xl text-debutron-navy dark:text-slate-200">Latest News &amp; Updates</h2>
			<div className="grid gap-8 md:grid-cols-3">
				{latestNews.map((item) => (
					<article key={item.id} className="overflow-hidden rounded-sm border border-gray-100 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
						...
						<div className="p-5">
							<h3 className="font-serif text-xl text-debutron-navy dark:text-slate-200">{item.title}</h3>
			...
			<div className="mt-8 text-center">
				<Link className="inline-block rounded-sm border border-debutron-navy px-5 py-2 font-sans text-sm font-semibold text-debutron-navy hover:bg-debutron-navy/10 dark:border-slate-500 dark:text-slate-200 dark:hover:bg-slate-700 dark:hover:text-slate-100" to="/news">
					View All News
				</Link>
			</div>
		</section>
